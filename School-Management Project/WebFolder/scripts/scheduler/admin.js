function initScheduler(containerID, date, view, syncObj) {
	...
	scheduler.attachEvent("onBeforeLightbox", function(event_id) {
		...
		if(typeof event_id !== 'string'){
			var dataSource = mappingObj.source,
				obj = mappingObj.getObject(event_object, true);

			dataSource.addNewElement();
			for(var attr in obj){
				if(dataSource.hasOwnProperty(attr) && attr != mappingObj.dc.getPrimaryKeyAttribute()){
					dataSource[attr] = obj[attr];
				}
			}
		}
